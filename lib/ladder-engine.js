(function (root, factory) {
  ...
  function parseScoreSummary(summary) {
    ...
    if (!raw) return { valid: false, sets: [], error: "Enter at least one completed set." };
    const parts = raw.split(/\s*,\s*/).filter(Boolean);
    const sets = [];

    for (const part of parts) {
      const match = part.match(/^(\d{1,2})\s*[-–]\s*(\d{1,2})$/);
      if (!match) return { valid: false, sets: [], error: `“${part}” is not a valid set score.` };
      const a = Number(match[1]);
      const b = Number(match[2]);
      if (a === b) return { valid: false, sets: [], error: "A completed set cannot end tied." };
      ...
      if (!standard && !proSet && !matchTiebreak) {
        return { valid: false, sets: [], error: `“${part}” does not look like a completed tennis set or match tiebreak.` };
      }
      sets.push({ a, b });
    }

    return { valid: true, sets, error: "" };
  }
