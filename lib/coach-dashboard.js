...
function buildPlayerSnapshots(data, playerMap, challengeMap) {
  ...
  return data.players.filter(player => player.active_status === "active").map(player => {
    ...
    const allOfficialForm = formMap.get(player.id) || [];
    const wins = allOfficialForm.filter(item => item.result === "W").length;
    const losses = allOfficialForm.filter(item => item.result === "L").length;
    return {
      id: player.id,
      ...
      officialForm: allOfficialForm.slice(0, 5),
