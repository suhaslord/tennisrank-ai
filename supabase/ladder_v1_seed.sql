-- TennisRank ladder initialization + repeatable spreadsheet synchronization.
-- Run after the base ladder migration.
--
-- This function is intentionally idempotent: the same imported singles order is a
-- no-op, while a changed imported order becomes the authoritative official ladder.
...
begin
  ...
  -- Resolve the incoming ordered roster first. This makes the function safe to
  -- call after every spreadsheet refresh instead of only once.
  ...
  loop
    ...
    if v_player_id is null then
      ...
      set profile_id = coalesce(profile_id, v_profile_id),
          display_name = v_name,
          active_status = case when active_status = 'injured' then 'injured' else 'active' end,
          updated_at = now()
  ...
  select coalesce(array_agg(player_id order by rank_position), array[]::uuid[])
    into v_current_ids
  ...
  -- Repeated Google Sheet refreshes with the same order must not disturb active
  -- challenges or write duplicate rank history.
  if not v_changed then
    return v_rank;
  end if;

  -- A changed authoritative import invalidates challenge positions based on the
  -- previous board. Cancel only open challenges for this team before re-ranking.
  update public.challenges
  set status = 'cancelled'
  where team_gender = p_team_gender
    and status in ('pending_response', 'accepted', 'scheduled', 'played', 'score_submitted', 'pending_coach_approval');
  get diagnostics v_cancelled = row_count;

  set constraints ladder_entries_gender_rank_unique deferred;

  -- Move existing rows out of the target rank range while preserving their old
  -- positions for rank history, then place the imported roster deterministically.
  update public.ladder_entries
  set previous_rank_position = rank_position,
      rank_position = rank_position + 10000,
      updated_at = now()
  where team_gender = p_team_gender;
  ...
  loop
    ...
    select previous_rank_position into v_old_rank
    from public.ladder_entries
    where player_id = v_player_id;

    if found then
      update public.ladder_entries le
      set team_gender = p_team_gender,
          rank_position = v_rank,
          status = case when p.active_status = 'injured' then 'injury_hold' else 'available' end,
          updated_at = now()
      from public.players p
      where le.player_id = v_player_id and p.id = le.player_id;
    else
      insert into public.ladder_entries (player_id, team_gender, rank_position, previous_rank_position, status)
      select p.id, p_team_gender, v_rank, v_rank,
             case when p.active_status = 'injured' then 'injury_hold' else 'available' end
      from public.players p
  ...
  where team_gender = p_team_gender
    and not (player_id = any(v_target_ids));
  ...
  values (
    p_coach_profile_id,
    'sync_ladder_from_import',
    'ladder',
    p_team_gender,
    jsonb_build_object(
      'player_count', array_length(v_target_ids, 1),
      'removed_count', v_removed,
      'cancelled_challenges', v_cancelled
    )
  );
