...
const encode=(rows,delimiter)=>rows.map(row=>row.map(v=>'"'+String(v).replaceAll('"','""')+'"').join(delimiter)).join('\r\n');
const layouts=[
 {name:'quoted CSV with multiline notes',text:encode([headers,...data],',')},
 {name:'BOM TSV with whitespace',text:'\ufeff'+encode([headers,...data.map(r=>r.map(v=>'  '+v+'  '))],'\t')},
 {name:'semicolon export',text:encode([headers,...data],';')},
 {name:'title blanks repeated headers',text:encode([['Team results'],[],headers,...data.slice(0,2),[],headers,...data.slice(2)],',')},
];
for(const width of [320,768,1440]) for(const fixture of layouts) {
 test(`${width}px ${fixture.name}: exact records, preview, publish, reload`,async({page})=>{
  await page.setViewportSize({width,height:900});
  const errors=[];page.on('pageerror',e=>errors.push(e.message));
  const state=await installImportSyncMocks(page);await openCsvImport(page);
  await page.locator('#csvText').fill(fixture.text);await page.locator('#useCsv').click();
  ...
  const table=page.locator('#rankingTable');
  await expect(table).toContainText('José O’Neil');
  await expect(table.locator('tr').filter({hasText:'José O’Neil'})).toContainText('1-1');
  await expect(table.locator('tr').filter({hasText:'Maya Long-Surname'})).toContainText('2-0');
  expect(await table.locator('tr').count()).toBe(4);
  expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1)).toBe(true);
  await page.reload();await expect(table).toContainText('Maya Long-Surname');
  await expect(table.locator('tr').filter({hasText:'Maya Long-Surname'})).toContainText('2-0');
