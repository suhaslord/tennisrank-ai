...
test(`${width}px shuffled history and mixed dates render correct charts`, async ({ page }) => {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (path === '/api/records') return reply({
      rows: [
        ...
        { Gender: 'Boys', Division: 'Singles', 'Player 1': 'Player QA', 'Player 2': 'Opponent QA', Winner: 'Player QA', Loser: 'Opponent QA', Score: '6-3', Date: '8/16/2026' },
        { Gender: 'Boys', Division: 'Singles', 'Player 1': 'Player QA', 'Player 2': 'Second QA', Winner: 'Second QA', Loser: 'Player QA', Score: '7-5', Date: 'Aug 15 2026' },
  ...
  await expect(page.locator('#playerStatGrid .player-stat').nth(2).locator('strong')).toHaveText('#2');
  expect(await page.locator('.rank-chart text').allTextContents()).toEqual(['#5','#4','#2']);
