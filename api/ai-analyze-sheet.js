...
function buildRedactedPayload(rows, sourceName, currentAnalysis = {}) {
  if (!Array.isArray(rows) || !rows.length) throw Object.assign(new Error("No spreadsheet rows were provided for AI analysis."), { status: 400 });
  ...
  const sourceRows = rows.slice(0, MAX_ROWS);
  const samples = sourceRows.map(row => Object.fromEntries(keys.map(key => [key, redact(row?.[key])])));
...
async function callGemini(apiKey, model, payload, signal) {
  ...
  catch { throw Object.assign(new Error("Gemini returned malformed structured output."), { status: 502 }); }
  return parsed;
}
...
function validateAiResult(result, inputKeys) {
  ...
  for (const item of mappings) {
    ...
    cleaned.push({ inputKey, target, confidence: Math.max(0, Math.min(1, confidence)), reason: cleanText(item?.reason, 220) });
  }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!apiKey) return json(res, 503, { error: "AI spreadsheet analysis is not configured yet.", code: "AI_NOT_CONFIGURED" });

    const body = parseBody(req);
    if (JSON.stringify(body).length > MAX_BODY_CHARS) return json(res, 413, { error: "This spreadsheet sample is too large for AI analysis." });
    const rows = Array.isArray(body.rows) ? body.rows : [];
    const payload = buildRedactedPayload(rows, body.sourceName, body.analysis || {});
    const requestedModel = cleanText(process.env.GEMINI_SPREADSHEET_MODEL || DEFAULT_MODEL, 80) || DEFAULT_MODEL;
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 12_000);
    try {
      const raw = await callGemini(apiKey, requestedModel, payload, controller.signal);
      const ai = validateAiResult(raw, payload.inputKeys);
      return json(res, 200, {
        ai,
        model: requestedModel,
        privacy: { redactedBeforeProvider: true, sampleRows: payload.samples.length, columns: payload.inputKeys.length },
      });
    } finally {
    ...
    return json(res, status >= 400 && status < 600 ? status : 500, { error: error.message || "Unexpected AI analysis error." });
  }
};

module.exports.buildRedactedPayload = buildRedactedPayload;
module.exports.validateAiResult = validateAiResult;
module.exports.sourceContext = sourceContext;
module.exports.ALLOWED_TARGETS = ALLOWED_TARGETS;
