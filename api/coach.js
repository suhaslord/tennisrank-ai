...
async function loadCoachData(context) {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      const data = await loadCoachData(context);
    ...
    return json(res, error.status || 500, { error: error.message || "Unexpected coach dashboard error." });
  }
};
