...
const { json, databaseConfig, serviceHeaders, allowApi, parseBody } = require("./_supabase");
...
module.exports = async function handler(req, res) {
  ...
  if (!setupToken || !safeEqual(req.headers["x-admin-token"], setupToken)) return json(res, 401, { error: "The admin setup code is invalid." });

  try {
    const existingResponse = await fetch(`${url}/rest/v1/profiles?role=eq.admin&select=id&limit=1`, { headers: serviceHeaders(key) });
    ...
    if (!authResponse.ok) return json(res, authResponse.status, { error: authPayload.msg || authPayload.message || authPayload.error || "Admin auth account could not be created." });
    const user = authPayload.user || authPayload;
    if (!user?.id) return json(res, 500, { error: "Supabase did not return an admin user ID." });

    const displayName = email.split("@")[0].replace(/[._-]+/g, " ").replace(/\b\w/g, char => char.toUpperCase()) || "Tennis Coach";
    ...
    if (!profileResponse.ok) {
      await fetch(`${url}/auth/v1/admin/users/${encodeURIComponent(user.id)}`, { method: "DELETE", headers: serviceHeaders(key) }).catch(() => {});
